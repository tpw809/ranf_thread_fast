...
class MetricFastener:
    ...
    @property
    def thread_tensile_stress_area_jis(self):
        """stress area in threaded portion, [mm^2]
        JIS method:
        d_outer =
        pitch =
    ...
    def d2(self):
        """
        [mm], pitch diameter of the thread  (based on JIS)
        d_outer =
        pitch =
        """
    ...
    def length_change(self, preload, lj):
        """
        [mm], change in bolt length due to tension
    ...
    def torque_for_tension(self, preload, k=0.2):
        """
        [N-mm], torque required for desired tension (preload)
    ...
    def thread_friction_torque_t2(self, mu, pb):
        """
        [N-mm], torque to overcome thread friction
    ...
    def washer_friction_torque_t3(self, mu: float, pb: float):
        """
        [N-mm], torque to overcome friction between bearing surfaces
    ...
    def nut_turns_for_tension(self, preload: float, lj: float, hn: float, hb: float):
        """
...
class M2MetricFastener(MetricFastener):
    def __init__(
        ...
        self.size = 'M2x0.4mm'
        # self.l_overall = 10.0  # [mm], length overall
        # self.l_shank = 4.0  # [mm], length of shank (unthreaded portion of length)
        self.pitch = 0.4  # [mm], distance between subsequent threads
        self.d_outer = 2.0  # [mm], major (outer) diameter
        self.d_head = 3.8  # [mm], head washer bearing diameter
        self.coarse = True
        self.sigma_u = 440.0 / 0.8  # [MPa], ultimate tensile strength
        self.sigma_y = 440.0  # [MPa], yield strength
        self.thread_angle = 60.0 * np.pi / 180.0  # [rad], thread angle
        self.clearance_hole_nom = 2.40  # [mm], nominal clearance hole
        self.clearance_hole_close = 2.20  # [mm], close clearance hole
        self.clearance_hole_loose = 2.60  # [mm], loose clearance hole
        self.tapped_hole = 1.7  # [mm], tapped hole size
...
class M3MetricFastener(MetricFastener):
    def __init__(self, l_overall=10.0, l_shank=4.0):
        super().__init__(l_overall=l_overall,
                         l_shank=l_shank)
        self.size = 'M3x0.5mm'
        # self.l_overall = 10.0  # [mm], length overall
        # self.l_shank = 4.0  # [mm], length of shank (unthreaded portion of length)
        self.pitch = 0.5  # [mm], distance between subsequent threads
        self.d_outer = 3.0  # [mm], major (outer) diameter
        self.d_head = 5.5  # [mm], head washer bearing diameter
        self.coarse = True
        self.sigma_u = 440.0 / 0.8  # [MPa], ultimate tensile strength
        self.sigma_y = 440.0  # [MPa], yield strength
        self.thread_angle = 60.0 * np.pi / 180.0  # [rad], thread angle
        self.clearance_hole_nom = 3.40  # [mm], nominal clearance hole
        self.clearance_hole_close = 3.20  # [mm], close clearance hole
        self.clearance_hole_loose = 3.60  # [mm], loose clearance hole
        self.tapped_hole = 2.7  # [mm], tapped hole size
...
class M4MetricFastener(MetricFastener):
    def __init__(self, l_overall=20.0, l_shank=5.0):
        super().__init__(l_overall=l_overall,
                         l_shank=l_shank)
        self.size = 'M4x0.7mm'
        # self.l_overall = 20.0  # [mm], length overall
        # self.l_shank = 8.0  # [mm], length of shank (unthreaded portion of length)
        self.pitch = 0.7  # [mm], distance between subsequent threads
        self.d_outer = 4.0  # [mm], major (outer) diameter
        self.d_head = 7.0  # [mm], head washer bearing diameter
        self.coarse = True
        self.sigma_u = 440.0 / 0.8  # [MPa], ultimate tensile strength
        self.sigma_y = 440.0  # [MPa], yield strength
        self.thread_angle = 60.0 * np.pi / 180.0  # [rad], thread angle
        self.clearance_hole_nom = 4.40  # [mm], nominal clearance hole
        self.clearance_hole_close = 4.20  # [mm], close clearance hole
        self.clearance_hole_loose = 4.60  # [mm], loose clearance hole
        self.tapped_hole = 3.7  # [mm], tapped hole size
...
class M5MetricFastener(MetricFastener):
    def __init__(self, l_overall=30.0, l_shank=10.0):
        super().__init__(l_overall=l_overall,
                         l_shank=l_shank)
        self.size = 'M5x0.8mm'
        # self.l_overall = 30.0  # [mm], length overall
        # self.l_shank = 10.0  # [mm], length of shank (unthreaded portion of length)
        self.pitch = 0.8  # [mm], distance between subsequent threads
        self.d_outer = 5.0  # [mm], major (outer) diameter
        self.d_head = 8.5  # [mm], head washer bearing diameter
        self.coarse = True
        self.sigma_u = 440.0 / 0.8  # [MPa], ultimate tensile strength
        self.sigma_y = 440.0  # [MPa], yield strength
        self.thread_angle = 60.0 * np.pi / 180.0  # [rad], thread angle
        self.clearance_hole_nom = 5.40  # [mm], nominal clearance hole
        self.clearance_hole_close = 5.20  # [mm], close clearance hole
        self.clearance_hole_loose = 5.60  # [mm], loose clearance hole
        self.tapped_hole = 4.7  # [mm], tapped hole size
...
class M6MetricFastener(MetricFastener):
    def __init__(self, l_overall=30.0, l_shank=10.0):
        ...
        # self.l_overall = 30.0  # [mm], length overall
        # self.l_shank = 10.0  # [mm], length of shank (unthreaded portion of length)
        self.pitch = 1.0  # [mm], distance between subsequent threads
        self.d_outer = 6.0  # [mm], major (outer) diameter
        self.d_head = 10.0  # [mm], head washer bearing diameter
        self.coarse = True
        self.sigma_u = 440.0 / 0.8  # [MPa], ultimate tensile strength
        self.sigma_y = 440.0  # [MPa], yield strength
        self.thread_angle = 60.0 * np.pi / 180.0  # [rad], thread angle
        self.clearance_hole_nom = 6.40  # [mm], nominal clearance hole
        self.clearance_hole_close = 6.20  # [mm], close clearance hole
        self.clearance_hole_loose = 6.60  # [mm], loose clearance hole
        self.tapped_hole = 5.7  # [mm], tapped hole size
...
class M10MetricFastener(MetricFastener):
    def __init__(self, l_overall=50.0, l_shank=20.0):
        super().__init__(l_overall=l_overall,
                         l_shank=l_shank)
        self.size = 'M10x1.5mm'
        ...
        # [mm], length of shank (unthreaded portion of length):
        # self.l_shank = 20.0
